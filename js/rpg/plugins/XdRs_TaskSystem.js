...
Game_Task.prototype.check = function(labels) {
    ...
    const sym = eval(parameters['filterType']) ? 'every' : 'some';
    return labels[sym]((n) => {
        if (n < 4) return this.status() === n;
        return this.type() === (n - 4);
    });
};
...
Game_Task.prototype.addCondition = function(data) {
    /* 记录任务条件的 type ，各种条件变量变更时依照含有该类型的条件来刷新，避免一次所有任务全部刷新 */
    this._types.push(data.type); 
    this._conditions.push(new Game_TaskCondition(this._id, data));
    this.refresh();
};
Game_Task.prototype.refresh = function(type) {
    const result = this._types.contains(type) || type === void 0;
    if (result && !this.isCompleted() && !this.isFailed()) {
        let result = false;
        ...
        result && $gameTemp.setTaskNeedRefresh(true);
    }
};
...
Game_Temp.prototype.setupTaskTypes = function() {
...
Game_System.prototype.initialize = function() {
    ...
    this._trackeData = {'tasks':[],'x':0,'y':200,'show':true,'visibility':true};
};
Game_System.prototype.trackedTasks = function() {
    return this._trackeData.tasks;
};
Game_System.prototype.isTaskTracked = function(task) {
    return task && this._trackeData.tasks.contains(task.id());
};
...
Game_System.prototype.trackeWindowVisibility = function() {
    return this._trackeData.visibility;
};
...
Game_Switches.prototype.setValue = function(switchId, value) {
    ...
    tmp !== this.value(switchId) && $gameParty.refreshTasks(2);
};
//=================================================================================================
...
Game_Variables.prototype.setValue = function(variableId, value) {
    ...
    tmp !== this.value(variableId) && $gameParty.refreshTasks(1);
};
//=================================================================================================
...
Game_Party.prototype.restartTask = function(taskId) {
    this.hasTask(taskId) && this.task(taskId).again();
};
Game_Party.prototype.refreshTasks = function(type) {
...
Window_Base.prototype.drawIcon = function(iconIndex, x, y, bw, bh) {
...
Window_TaskList.prototype.drawItem = function(index) {
    ...
    if (item) {
        ...
        if ($gameSystem.isTaskTracked(item)) {
...
Window_TaskList.prototype.processOk = function() {
    ...
    const type = $gameSystem.isTaskTracked(task) ? 1 : 0;
...
Window_TrackeList.prototype.drawStyle = function() {
    ...
    if (!this._uiSprite) {
        ...
        const frameColor = parameters['frameColor'];
        for (let i=0;i<this._nums;++i) {
            this.drawRoundRect(0, h * i + 2, w, h-4, 5, 0, '', frameColor);
        }
    }
};
...
Scene_Task.prototype = Object.create(Scene_Base.prototype);
...
Scene_Task.prototype.setBackgroundOpacity = function(opacity) {
    Scene_MenuBase.prototype.setBackgroundOpacity.call(this, opacity);
};
...
Scene_Map.prototype.createTaskWindow = function() {
    if ($gameSystem.trackeWindowVisibility()) {
