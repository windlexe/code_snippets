...
Game_Temp.prototype.initialize = function(bitmap){
	...
	this._drill_COBa_opacityData = {};		//临时容器 - 透明度弹道
	this._drill_COBa_rotateData = {};		//临时容器 - 旋转角弹道
}
...
Game_Temp.prototype.drill_COBa_setBallisticsOpacity = function( data ){
	...
	//   随机因子（RandomFactor）
	if( data['opacityRandomFactor'] == undefined ){ data['opacityRandomFactor'] = -1 };		//透明度 - 随机因子（锁定随机值专用,0-1之间）
...
Game_Temp.prototype.drill_COBa_preBallisticsOpacity = function( obj_data, obj_index, orgOpacity ){
	...
	if( isNaN( orgOpacity ) == true && DrillUp._drill_COBa_checkOn == true ){
		DrillUp._drill_COBa_checkOn = false;
		alert(
			"【Drill_CoreOfBallistics.js 系统 - 弹道核心】"+
			"\n检测到orgOpacity出现NaN值，请及时检查你的函数。"
		);
	}
	...
	/*-----------------时间公式模式------------------*/
	if( data['opacityMode'] == "时间公式模式"){
		
		// > 起点值
		obj_data['_drill_COBa_opacity'].push( orgOpacity );
		
		// > 随机值（只有随机值和时间没有关系）
		var randomValue = Math.random();
		if( data['opacityRandomFactor'] != -1 ){ randomValue = data['opacityRandomFactor']; }
		...
		for(var time=1; time < data['opacityTime']; time++){
			...
			p.time  = time;
			p.ran   = randomValue;
			p.num   = data['opacityNum'];
			
			oo = data['otFunction'].call(this, p );	
			DrillUp.drill_COBa_checkValue( p, oo );		//（校验）
		
			oo = orgOpacity + oo;
			obj_data['_drill_COBa_opacity'].push(oo);
		}
	}
	
	
	/*-----------------时间锚点模式------------------*/
	if( data['opacityMode'] == "时间锚点模式"){
		...
		for(var time = 1; time <= data['opacityTime']; time++){
			var time_per = time * 100 / data['opacityTime'];	//（时间百分比） 
			...
			// > 直接找到末尾点
			if( end_index == 0 ){
				obj_data['_drill_COBa_opacity'].push( data['anchorPointTank'][ data['anchorPointTank'].length-1 ] );
				continue;
			}
			// > 开头点都没接触到
			if( start_index == -1 ){
				obj_data['_drill_COBa_opacity'].push( data['anchorPointTank'][0] );
				continue;
			}
			...
			var oo = cur_time / d_time * ( p_end['o'] - p_start['o'] ) + p_start['o'];
			obj_data['_drill_COBa_opacity'].push( oo );
		}
	...
	if( data['opacityMode'] == "目标值模式"){
		...
		for(var time = 0; time <= data['opacityTime']; time++){
			// > 透明度
			oo = 0;
			
			if( data['targetType'] == "瞬间变化"){
				oo = data['targetDifference'];
			}
			
			if( data['targetType'] == "匀速变化"){
				dO = data['targetDifference'];
				dt = data['opacityTime'];
			...
			if( data['targetType'] == "增减速变化"){	
				...
				var v_max = dO / dt *2;			//（先加速后减速）
				var a = v_max / dt *2;
				if( time < dt/2 ){
					oo = a*time*time/2;
				}else{
					var t_p = time - dt/2;
					oo = dO/2 + v_max*t_p - a*t_p*t_p/2;
				}
			}
			...
			if( data['targetType'] == "弹性变化"){
				...
				var ao = 2 * dO / dt / dt;		//（匀减速移动到目标点）
				var c_time = dt - time;
				oo = 0.5 * ao * dt * dt - 0.5 * ao * c_time * c_time ;
			}
	...
	// > 延迟（开始前延迟时间）
	for(var i = 0; i < data['opacityDelay']; i++){
		obj_data['_drill_COBa_opacity'].unshift( obj_data['_drill_COBa_opacity'][0] );
	}
	// > 延迟（到终点后延迟时间）
	for(var i = 0; i < data['opacityEndDelay']; i++){
		obj_data['_drill_COBa_opacity'].push( obj_data['_drill_COBa_opacity'][ obj_data['_drill_COBa_opacity'].length-1 ] );
	}
