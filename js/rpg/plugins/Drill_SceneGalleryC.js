...
//==============================
	...
	if( DrillUp.global_SGaC_enableTank == null ){	//（游戏没关时，不会为null)
		if( _drill_global && _drill_global[0] && _drill_global[0]["_global_Drill_SGaC_enable"] ){		//游戏关闭后，重开读取global中的配置
			DrillUp.global_SGaC_enableTank = _drill_global[0]["_global_Drill_SGaC_enable"];
		}else{
			DrillUp.global_SGaC_enableTank = [];		//（若没有全局存储的值，则初始化）
			for( var i = 1; i <= DrillUp.g_SGaC_context_list_length ; i++ ){
	...
	if( DrillUp.global_SGaC_lockTank == null ){	
		if( _drill_global && _drill_global[0] && _drill_global[0]["_global_Drill_SGaC_lock"] ){
			DrillUp.global_SGaC_lockTank = _drill_global[0]["_global_Drill_SGaC_lock"];
		}else{
			DrillUp.global_SGaC_lockTank = [];			//（若没有全局存储的值，则初始化）
			for( var i = 1; i <= DrillUp.g_SGaC_context_list_length ; i++ ){
				var temp_c = DrillUp.g_SGaC_context_list[i];
				if( temp_c == null ){ continue; }
...
// * 全局 - 存储
//==============================
var _drill_SGaC_saveGlobal = DataManager.saveGlobalInfo;
DataManager.saveGlobalInfo = function(info) {	//第0个存档为全局存档
	if(!info[0]){info[0] = []};
	info[0]["_global_Drill_SGaC_enable"] = DrillUp.global_SGaC_enableTank;
	info[0]["_global_Drill_SGaC_lock"] = DrillUp.global_SGaC_lockTank;
	_drill_SGaC_saveGlobal.call(this,info);
};
...
Game_System.prototype.initialize = function() {	
	...
	this._drill_SGaC_context_list = null;		//旧存档数据
	this._drill_SGaC_enableTank = null;			//显示情况
	this._drill_SGaC_lockTank = null;			//锁定情况
};
//==============================
// * 正常 - 读取 正常存储 的值
//==============================
Game_System.prototype.drill_SGaC_initDataIfNeed = function() {
	if( this._drill_SGaC_enableTank != null ){ return; }
	
	// > 一般初始化
	if( this._drill_SGaC_context_list == null ){
		
		this._drill_SGaC_enableTank = [];
		for( var i = 1; i <= DrillUp.g_SGaC_context_list_length ; i++ ){
			var temp_c = DrillUp.g_SGaC_context_list[i];
			if( temp_c == null ){ continue; }
			this._drill_SGaC_enableTank[i] = temp_c['enabled'];
		}
		this._drill_SGaC_lockTank = [];
		for( var i = 1; i <= DrillUp.g_SGaC_context_list_length ; i++ ){
			var temp_c = DrillUp.g_SGaC_context_list[i];
			if( temp_c == null ){ continue; }
	...
	// > 从旧存档中初始化
		...
		this._drill_SGaC_enableTank = [];
		for( var i = 1; i <= DrillUp.g_SGaC_context_list_length ; i++ ){
			var temp_c = this._drill_SGaC_context_list[i];
			if( temp_c == null ){ continue; }
			if( temp_c == "" ){ continue; }
			this._drill_SGaC_enableTank[i] = temp_c['enabled'];
		}
		...
		this._drill_SGaC_context_list = null;		//（强制清除旧插件的冗余存档数据）
	}
};
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	...
	if( command === ">画廊C" ){
		$gameSystem.drill_SGaC_initDataIfNeed();	//（初始化）
		if(args.length == 2){
	...
	if( command === ">画廊C" ){
		...
		if(args.length == 4){
			...
			if( type == "选中页" ){
				...
				$gameSystem._drill_SGaC_context_index = temp;
			}
			if( type == "打开面板(单图查看模式)" ){
...
Scene_Drill_SGaC.prototype.initialize = function() {
	...
	this._cur_index = -1;
	$gameSystem.drill_SGaC_initDataIfNeed();
};
//==============================
// * 画廊C - 创建
//============================== 
...
Scene_Drill_SGaC.prototype.drill_updateIndex = function() {
	if( $gameSystem._drill_SGaC_context_index != undefined || $gameSystem._drill_SGaC_context_index != null ){
		this._window_select.select( $gameSystem._drill_SGaC_context_index );
		$gameSystem._drill_SGaC_context_index = null;		//设置选中页
	}
