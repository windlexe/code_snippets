 ...
 *
 * @param SkipSwitchId
 * @desc 如果指定的号码的开关是ON的情况下经常跳过。
 * @default 0
 * @type switch
 *
 * @param AutoSwitchIId
 * @desc 指定号码的开关是ON的情况下经常自动。优先跳过。
 * @default 0
 * @type switch
 *
 * @param SkipIcon
 * @desc 在消息跳过过程中显示在窗口右下方的图标
 * @default 140
 * @type number
 *
 * @param AutoIcon
 * @desc 在消息自动中显示在窗口右下方的图标
 * @default 75
 * @type number
 *
 * @param PressingSkip
 * @desc 跳过的判定只在按指定的键的时候。
 * @default false
 * @type boolean
 *
 * @param AutoWaitFrame
 * @desc 在自动模式有效的情况下预先显示消息的帧数。控制字符\v[n]以及可以指定计算公式。
 * @default 100 + textSize * 10
 *
 * @param ResetOnEndSwitch
 * @desc 指定的号码的开关是ON的时候，事件结束的时候跳过，解除自动状态。0的情况经常解除。
 * @default 0
 * @type switch
 *
 * @param SkipPicture
 * @desc 是窗口内显示的跳转图片的文件名。点击进入跳转模式。
 * @default
 * @require 1
 * @dir img/pictures/
 * @type file
 *
 * @param PictureAnchor
 * @desc 是跳跃，自动的各图片按钮坐标的原点。（0:左上、1:右上、2:左下、3:右下）
 * @default 0
 * @type select
 * @option 0
 * @option 1
 ...
 * @desc 指定的号码的开关是ON的时候跳过，自动的各图片按钮显示。0的情况无条件表示。
 * @default 0
 * @type switch
 *
 * @param SkipPictureX
 ...
 * @desc 指定的开关是ON的时候插件的所有功能都无效。
 * @default 0
 * @type switch
 *
 * @help 在消息窗口可以跳过消息或切换自动模式。
 * 活动结束的话自动跳过和自动模式被解除。
 * 并行执行的事件，只有在正常事件没有执行的情况下才会被解除。
 * 如果想要明确地解除，请执行以下脚本。
 *
 * $gameMessage.clearSkipInfo();
 *
 * ・SkipAlreadyReadMessage.jsと的合作
 * SkipAlreadyReadMessage.js和(奏)一起使用的时候
 * 本插件的跳过功能成为“已读跳过”功能。
 * http://makonet.sakura.ne.jp/rpg_tkool/
 *
 * ・设定参数“自动待机帧”的话可以变更自动模式时的待机帧。
 * 操纵字符\v[n]除此之外，还可以使用JavaScript计算公式。
 * 并且可以用textSize将显示文字数并入计算公式中。
 *
 * 例：
 * 100 + textSize * 10
 *
